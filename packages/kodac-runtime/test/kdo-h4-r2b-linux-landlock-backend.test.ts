...
test("launch-plan authority rejects unsupported modes PATH resolution ambiguity and unsafe grants", () => {
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "danger-full-access" as never, readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "launcher", mode: "read-only", readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "read-only", readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["node"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "workspace-write", readOnlyRoots: ["/"], readWriteRoots: ["/z", "/a"], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "workspace-write", readOnlyRoots: ["/", "/workspace"], readWriteRoots: ["/workspace"], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "read-only", readOnlyRoots: ["/"], readWriteRoots: ["/tmp"], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "workspace-write", readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["/bin/true"] }))
})
...
test("launch-plan inputs are bounded strict and reject structural hooks without executing them", () => {
  ...
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "workspace-write", readOnlyRoots: ["/"], readWriteRoots: tooManyRoots, targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: `/${"界".repeat(1400)}`, mode: "read-only", readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["/bin/true"] }))
  assert.throws(() => createLinuxLandlockLaunchPlan({ launcherPath: "/launcher", mode: "read-only", readOnlyRoots: ["/"], readWriteRoots: [], targetArgv: ["/bin/true", "界".repeat(22000)] }))

  let traps = 0
  const proxied = new Proxy({ launcherPath: "/launcher", mode: "read-only" as const, readOnlyRoots: ["/"], readWriteRoots: [] as string[], targetArgv: ["/bin/true"] }, {
...
test("probe classification is exact bounded and never upgrades partial or malformed evidence", () => {
  const full = classifyLinuxLandlockProbe({ exitCode: 0, stdout: "kodac-landlock-v1 abi=9 claim-set=kodac-linux-landlock-fs-v1 enforcement=full\n", stderr: "" })
  assert.equal(full.observedAbi, 9)
  assert.equal(full.enforcement, "full")

  const partial = classifyLinuxLandlockProbe({ exitCode: 0, stdout: "kodac-landlock-v1 abi=3 claim-set=kodac-linux-landlock-fs-v1 enforcement=partial\n", stderr: "" })
  assert.equal(partial.observedAbi, 3)
  assert.equal(partial.enforcement, "partial")

  assert.equal(classifyLinuxLandlockProbe({ exitCode: 125, stdout: "", stderr: "kodac-landlock: unavailable\n" }).enforcement, "unavailable")
  assert.equal(classifyLinuxLandlockProbe({ exitCode: 0, stdout: "kodac-landlock-v1 abi=3 claim-set=kodac-linux-landlock-fs-v1 enforcement=full\n", stderr: "" }).enforcement, "unavailable")
  assert.equal(classifyLinuxLandlockProbe({ exitCode: 0, stdout: "malformed\n", stderr: "" }).enforcement, "unavailable")
  assert.equal(classifyLinuxLandlockProbe({ exitCode: 0, stdout: `${"x".repeat(5000)}\n`, stderr: "" }).enforcement, "unavailable")
})
...
linuxOnly("Linux native launcher compiles probes and enforces read-only/workspace-write file effects", () => {
  ...
  try {
    ...
    const relativeTarget = spawnSync(binary, ["--ro", "/", "--", "node", "-e", "process.stdout.write('TARGET_EXECUTED')"], { encoding: "utf8", shell: false })
    ...
    const classification = classifyLinuxLandlockProbe({ exitCode: probe.status ?? 255, stdout: probe.stdout, stderr: probe.stderr })
    assert.notEqual(classification.enforcement, "unavailable")
    assert.ok((classification.observedAbi ?? 0) >= 1)
    console.log(`H4_R2B_LANDLOCK_PROBE ${JSON.stringify({ abi: classification.observedAbi, enforcement: classification.enforcement, claimSet: classification.claimSet })}`)
    ...
    const read = spawnSync(binary, ["--ro", "/", "--", process.execPath, "-e", "require('node:fs').readFileSync('/etc/hosts')"], { encoding: "utf8", shell: false })
    ...
    const readOnlyWrite = spawnSync(binary, ["--ro", "/", "--", process.execPath, "-e", `require('node:fs').writeFileSync(${JSON.stringify(readOnlyDeniedFile)}, 'denied')`], { encoding: "utf8", shell: false })
    ...
    const workspaceWrite = spawnSync(binary, ["--ro", "/", "--rw", allowed, "--", process.execPath, "-e", `require('node:fs').writeFileSync(${JSON.stringify(allowedFile)}, 'allowed')`], { encoding: "utf8", shell: false })
    ...
    const siblingWrite = spawnSync(binary, ["--ro", "/", "--rw", allowed, "--", process.execPath, "-e", `require('node:fs').writeFileSync(${JSON.stringify(siblingDeniedFile)}, 'denied')`], { encoding: "utf8", shell: false })
    ...
    const setupFailure = spawnSync(binary, ["--ro", "/", "--rw", join(root, "missing"), "--", process.execPath, "-e", "process.stdout.write('TARGET_EXECUTED')"], { encoding: "utf8", shell: false })
