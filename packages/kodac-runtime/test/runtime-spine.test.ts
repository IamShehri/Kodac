...
test("RuntimeSession emits versioned monotonic canonical events", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ["kodac.event", 1, "session-test", 1, "session.started"],
      ["kodac.event", 1, "session-test", 2, "tool.started"],
    ],
  )
})
...
test("CLI applies a patch through the canonical runtime spine and persists proof", async () => {
  ...
  const receiptRows = lines(await readFile(join(evidence, sessions[0], "receipts.jsonl"), "utf8")) as Array<{ result: { status: string } }>

  assert.deepEqual(
    eventRows.map((event) => event.type),
    [
      "session.started",
      "tool.started",
      "intent.created",
