...
function procStat(input: { policy?: number; rtPriority?: number; startTicks?: string } = {}): string {
  ...
  fields[18] = input.startTicks ?? START_TICKS
  fields[36] = String(input.rtPriority ?? 0)
  fields[37] = String(input.policy ?? 0)
...
function createSnapshot(requirement = fixtureRequirement(), mutate?: (raw: any) => void) {
  return createGvisorCgroupV2PhysicalResourceSnapshot({
    requirement,
    expectedPid: PID,
    expectedStartTicks: START_TICKS,
    raw: rawSnapshot(requirement, mutate),
  })
}

test("H4-R3G-A constants and non-root controller hierarchy are exact", () => {
...
test("H4-R3G-A synthetic success proves exact CPU memory no-swap E3 snapshot without root controller files", () => {
  ...
  assert.equal(snapshot.startTicks, START_TICKS)
  ...
  assert.equal(snapshot.effectiveCpuNumerator, "3")
  assert.equal(snapshot.effectiveCpuDenominator, "2")
  assert.equal(snapshot.availableCpuCount, 4)
  assert.equal(snapshot.effectiveMemoryBytes, String(requirement.workload.resourcePolicy.memoryBytes))
  assert.equal(snapshot.effectiveSwapBytes, "0")
  assert.equal(snapshot.schedulerPolicy, 0)
  assert.equal(snapshot.rtPriority, 0)
  assert.equal(snapshot.processCpusAllowed, "0-3")
  assert.deepEqual(validateGvisorCgroupV2PhysicalResourceSnapshot(snapshot), snapshot)
  assert.equal(Object.isFrozen(snapshot), true)
  assert.equal(Object.isFrozen(snapshot.levels), true)
  assert.match(createGvisorCgroupV2ObserverProtocolIdentity(), /^[0-9a-f]{64}$/)
})
...
test("H4-R3G-A snapshot validator re-derives normalized evidence rather than trusting caller fields", () => {
  const snapshot: any = JSON.parse(JSON.stringify(createSnapshot()))
  snapshot.effectiveCpuNumerator = "999"
  assert.throws(() => validateGvisorCgroupV2PhysicalResourceSnapshot(snapshot), /effective CPU ratio/)

  const memory: any = JSON.parse(JSON.stringify(createSnapshot()))
  memory.effectiveMemoryBytes = "1"
  assert.throws(() => validateGvisorCgroupV2PhysicalResourceSnapshot(memory), /effective memory/)

  const hierarchy: any = JSON.parse(JSON.stringify(createSnapshot()))
  hierarchy.hierarchyIdentity = "f".repeat(64)
  assert.throws(() => validateGvisorCgroupV2PhysicalResourceSnapshot(hierarchy), /hierarchy identity/)

  const count: any = JSON.parse(JSON.stringify(createSnapshot()))
  count.availableCpuCount = KDO_H4_R3G_A_LIMITS.maxCpuId + 2
  assert.throws(() => validateGvisorCgroupV2PhysicalResourceSnapshot(count), /availableCpuCount/)
})

test("H4-R3G-A runtime config is narrow and cannot carry path or reader authority", () => {
  const commitResourceEvidence = () => ({})
  const config = validateGvisorCgroupV2RuntimeConfig({ version: KDO_H4_R3G_A_RUNTIME_CONFIG_VERSION, commitResourceEvidence })
  assert.equal(config.commitResourceEvidence, commitResourceEvidence)
  assert.throws(() => validateGvisorCgroupV2RuntimeConfig({ version: KDO_H4_R3G_A_RUNTIME_CONFIG_VERSION, commitResourceEvidence, cgroupRoot: "/tmp/fake" }))
  assert.throws(() => validateGvisorCgroupV2RuntimeConfig({ version: KDO_H4_R3G_A_RUNTIME_CONFIG_VERSION, commitResourceEvidence, readFile() {} }))
})
...
test("H4-R3G-A production module is pure E3-only and protected R3B/R3E/R3F surfaces remain unchanged", () => {
  ...
  assert.match(moduleSource, /derivedHierarchy/)
  assert.match(moduleSource, /finiteRatio/)
  assert.match(moduleSource, /finiteLimit/)
  assert.match(moduleSource, /prePhysicalSnapshotIdentity/)
