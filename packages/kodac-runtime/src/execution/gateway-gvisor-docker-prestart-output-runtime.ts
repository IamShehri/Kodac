...
export class GvisorDockerPrestartOutputGateway {
  ...
  async preparePrestartOutput(permitValue: unknown, createdValue: unknown, createdCommitValue: unknown, options: { readonly signal?: AbortSignal } = {}): Promise<SandboxPrestartPreparationResult> {
    ...
    try {
      ...
      try { preparationRaw = await this.#runtime.commitPreparationTransaction({ prepared, preparedCommit, fence: preparedFence }, { signal: controller.signal }) } catch (error) { throw new SandboxPrestartIndeterminateError(`B2A preparation transaction is indeterminate: ${error instanceof Error ? error.message : String(error)}`) }
      let preparation
      ...
      try { claimRaw = await this.#runtime.commitOwnershipClaimTransaction({ claim, claimCommit, expectedFence: preparation.fence, nextFence: ownerFence }, { signal: controller.signal }) } catch (error) { throw new SandboxPrestartIndeterminateError(`B2A owner claim transaction is indeterminate: ${error instanceof Error ? error.message : String(error)}`) }
      let claimSettlement
      ...
      const preAttachFence = await exactFence(this.#runtime, prepared, controller.signal); if (preAttachFence.state === "FAILED_TERMINAL") throw new SandboxPrestartTerminalError("B2A operation became terminal before attach"); if (preAttachFence.state !== "OWNER_CLAIMED" || preAttachFence.ownershipClaimIdentity !== claim.ownershipClaimIdentity || preAttachFence.ownerInstanceIdentity !== claim.ownerInstanceIdentity) throw new SandboxPrestartIndeterminateError("B2A durable owner changed before attach")
      try { await revalidatePristineDormant(this.#runtime, permit, created, controller.signal) } catch { return await fail(this.#runtime, prepared, ownerFence, claim, ownerCapability, "attaching", "dormant-revalidation-failed") }
