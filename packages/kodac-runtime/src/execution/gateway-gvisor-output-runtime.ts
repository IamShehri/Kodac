...
import { createReceipt } from "../evidence/receipt.ts"
...
export interface GvisorOutputRuntimeConfig {
  ...
  /** Must atomically create-once by executionAttemptIdentity. Duplicate/conflict returns `exists` or throws. */
...
function validateJsonSyntaxNoDuplicateKeys(text: string): void {
  ...
  const stringToken = (): string => {
    ...
    while (index < length) {
      ...
      if (char === '"') { index += 1; try { return JSON.parse(text.slice(start, index)) as string } catch { throw new TypeError("R3G-E Docker inspect contains invalid JSON string syntax") } }
      if (char === "\\") {
        index += 1; if (index >= length) throw new TypeError("R3G-E Docker inspect contains unterminated JSON escape")
        if (text[index] === "u") { if (!/^[0-9a-fA-F]{4}$/.test(text.slice(index + 1, index + 5))) throw new TypeError("R3G-E Docker inspect contains invalid JSON unicode escape"); index += 4 }
  ...
  const value = (depth: number): void => {
    ...
    if (char === "{") {
      index += 1; skip(); const keys = new Set<string>(); if (text[index] === "}") { index += 1; return }
...
function parseInspectBody(body: Buffer, expectedContainerId: string): void {
  ...
  let parsed: unknown; try { parsed = JSON.parse(text) } catch { throw new TypeError("R3G-E Docker inspect is not valid JSON") }
  ...
  try {
    return await new Promise<Buffer>((resolve, reject) => {
      ...
      const request = httpRequest({ method: "GET", socketPath: input.socketPath, path: `/v${KDO_H4_R3G_E_DOCKER_API_VERSION}/containers/${input.containerId}/json?size=0`, agent: false, maxHeaderSize: KDO_H4_R3F_LIMITS.maxResponseHeaderBytes, headers: Object.freeze({ Accept: "application/json", Connection: "close" }) }, (response) => {
        ...
        response.on("data", (chunk: Buffer | string) => { const part = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk); bytes += part.byteLength; if (bytes > MAX_INSPECT_BYTES) { const error = new Error("R3G-E Docker inspect body exceeds bound"); finishReject(error); response.destroy(error); return }; chunks.push(part) })
        response.on("end", finishResolve); response.on("aborted", () => finishReject(new Error("R3G-E Docker inspect response aborted"))); response.on("error", finishReject)
      })
      request.on("error", finishReject); request.setTimeout(KDO_H4_R3F_LIMITS.requestTimeoutMs, () => { const error = new Error("R3G-E Docker inspect timed out"); finishReject(error); request.destroy(error) })
      input.signal?.addEventListener("abort", onAbort, { once: true }); if (input.signal?.aborted) { onAbort(); return }; request.end()
    })
  ...
  return await new Promise((resolve, reject) => {
    ...
    request.once("upgrade", (response, socketValue, head) => {
      try {
        ...
        requireSameSocketEndpoint(input.socketPath, input.endpoint)
        if (settled) { socket.destroy(); return }; settled = true; cleanup(); resolve(Object.freeze({ socket, head: Buffer.from(head) }))
      } catch (error) { socketValue.destroy(); finishReject(error) }
    })
    request.on("error", finishReject); request.setTimeout(KDO_H4_R3F_LIMITS.requestTimeoutMs, () => { const error = new Error("R3G-E Docker attach handshake timed out"); finishReject(error); request.destroy(error) })
    input.signal?.addEventListener("abort", onAbort, { once: true }); if (input.signal?.aborted) { onAbort(); return }; request.end()
  })
}

function sameBinding(left: GvisorContainerBinding, right: GvisorContainerBinding): boolean {
  return left.bindingIdentity === right.bindingIdentity && left.executionAttemptIdentity === right.executionAttemptIdentity && left.requirementIdentity === right.requirementIdentity && left.workloadIdentity === right.workloadIdentity && left.containerId === right.containerId && left.providerId === right.providerId
}
...
export function createGvisorDockerOutputTransport(value: unknown): GvisorDockerOutputTransport {
  ...
  const provider = createDockerControlPlaneBindingProvider(config); const consumedAttempts = new Set<string>()
  const captureOutput = async (requestValue: GvisorContainerBindingRequest, options: { readonly signal?: AbortSignal; readonly expectedBinding?: GvisorContainerBinding } = {}): Promise<GvisorDockerOutputCapture> => {
    ...
    const resolution = await provider.resolveDockerControlPlaneBinding(request, { signal: options.signal }); const binding = resolution.binding
    ...
    const accumulator = new GvisorDockerMultiplexAccumulator(config.requirement.workload.resourcePolicy.maxOutputBytes); let abortListener: (() => void) | undefined
    try {
      socket.setTimeout(Math.max(KDO_H4_R3F_LIMITS.requestTimeoutMs, config.requirement.workload.resourcePolicy.ttlMs), () => socket.destroy(new Error("R3G-E Docker output stream stalled beyond bounded lifetime")))
      if (options.signal !== undefined) { abortListener = () => socket.destroy(new Error("R3G-E Docker output capture aborted")); options.signal.addEventListener("abort", abortListener, { once: true }); if (options.signal.aborted) abortListener() }
      ...
      const aggregation = accumulator.finish(); requireSameSocketEndpoint(config.socketPath, provider.socketEndpoint)
      return Object.freeze({ version: KDO_H4_R3G_E_DOCKER_TRANSPORT_VERSION, binding, executionAttemptIdentity: request.executionAttemptIdentity, requirementIdentity: request.requirementIdentity, workloadIdentity: request.workloadIdentity, providerIdentity: provider.providerIdentity, socketEndpointIdentity: provider.socketEndpoint.endpointIdentity, outputChannelIdentity, mediaType: KDO_H4_R3G_E_ATTACH_MEDIA_TYPE, aggregation })
    } catch (error) { socket.destroy(); throw error }
    finally { if (abortListener !== undefined) options.signal?.removeEventListener("abort", abortListener); socket.destroy() }
  }
...
export function createGvisorOutputPreparedOperation(input: Omit<GvisorOutputPreparedOperation, "version" | "preparedIdentity">): GvisorOutputPreparedOperation {
  const base = Object.freeze({
    ...
    runtimeInstanceIdentity: identity(input.runtimeInstanceIdentity, "runtimeInstanceIdentity"),
    providerIdentity: identity(input.providerIdentity, "providerIdentity"),
    socketEndpointIdentity: identity(input.socketEndpointIdentity, "socketEndpointIdentity"),
    outputChannelIdentity: identity(input.outputChannelIdentity, "outputChannelIdentity"),
    outputOperationIdentity: identity(input.outputOperationIdentity, "outputOperationIdentity"),
    armRecordIdentity: identity(input.armRecordIdentity, "armRecordIdentity"),
    maxOutputBytes: boundedPositiveInteger(input.maxOutputBytes, 16_777_216, "maxOutputBytes"),
  })
...
async function boundedDurable<T>(label: string, signal: AbortSignal | undefined, operation: () => Promise<T> | T): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => { timer = setTimeout(() => reject(new Error(`${label} timed out`)), KDO_H4_R3G_E_RUNTIME_LIMITS.durableCallbackTimeoutMs) })
  const abort = signal === undefined ? new Promise<never>(() => {}) : new Promise<never>((_, reject) => { abortHandler = () => reject(new Error(`${label} aborted`)); signal.addEventListener("abort", abortHandler, { once: true }); if (signal.aborted) abortHandler() })
...
function classifyFailure(error: unknown, signal?: AbortSignal): GvisorOutputFailureReason {
  if (error instanceof GvisorOutputLimitExceededError) return "output-limit-exceeded"
...
export class GvisorOutputExecutionGateway extends ExecutionGateway {
  ...
  async enforceGvisorOutputBound(requirement: SandboxExecutionRequirement, observer?: ExecutionObserver, options: { readonly signal?: AbortSignal } = {}): Promise<GvisorOutputEnforcementResult> {
    const startedAt = new Date().toISOString(); const intent = outputIntent(requirement)
    await observer?.onIntent?.(intent)
    const policy = validatePolicyResult(await this.outputPolicy.evaluate(intent)); await observer?.onPolicy?.(policy)
    if (policy.decision !== "allow") {
      ...
      await observer?.onReceipt?.(receipt); throw new ExecutionBlockedError(receipt)
    }
    if (process.platform !== "linux") throw new ExecutionFailedError("R3G-E physical output enforcement requires Linux")
    if (requirement.requiredSemanticRuntimeClass !== "gvisor") throw new ExecutionFailedError("R3G-E requires requiredSemanticRuntimeClass=gvisor")
    if (this.outputTransport.provider.requirementIdentity !== requirement.requirementIdentity || this.outputTransport.provider.workloadIdentity !== requirement.workload.workloadIdentity) throw new ExecutionFailedError("R3G-E output transport is bound to a different requirement/workload")
    if (options.signal?.aborted) throw new ExecutionFailedError("R3G-E output enforcement aborted before lifecycle start")

    const subjectReady = deferred<GvisorTtlSubjectBinding>(); const armReady = deferred<GvisorTtlArmRecord>()
    ...
    const wrappedRuntime: GvisorTtlRuntimeConfig = Object.freeze({
      ...
      watchdogPath: originalRuntime.watchdogPath,
      expectedWatchdogSha256: originalRuntime.expectedWatchdogSha256,
      registryRoot: originalRuntime.registryRoot,
      resolveSubject: async (requirementValue, runtimeOptions) => { const subject = validateGvisorTtlSubjectBinding(await originalRuntime.resolveSubject(requirementValue, runtimeOptions), requirementValue); subjectReady.resolve(subject); return subject },
      commitPreparedIntent: (prepared) => originalRuntime.commitPreparedIntent(prepared),
      commitArmEvidence: async (recordValue) => {
        ...
        armReady.resolve(arm); return raw
      },
      commitTerminalEvidence: (terminal) => originalRuntime.commitTerminalEvidence(terminal),
    })
    ...
    const outputAbort = new AbortController(); let callerAbort: (() => void) | undefined
    ...
    void lifecycle.catch(() => outputAbort.abort())
    ...
    try {
      ...
      const reservation = validateGvisorOutputReservation(await boundedDurable("R3G-E durable output reservation", options.signal, () => this.outputRuntime.reserveOutputOperation(prepared as GvisorOutputPreparedOperation)), prepared)
      ...
      try { capture = await this.outputTransport.captureOutput(bindingRequest, { signal: outputAbort.signal, expectedBinding: subject.binding }) }
        ...
        const failure = createGvisorOutputFailureRecord(prepared, classifyFailure(error, options.signal), error)
        const rawCommit = await boundedDurable("R3G-E durable output failure", undefined, () => this.outputRuntime.commitFailureEvidence(failure))
        validateGvisorOutputFailureCommit(rawCommit, failure)
      ...
      try { lifecycleResult = await lifecycle }
        ...
        const rawCommit = await boundedDurable("R3G-E durable lifecycle failure", undefined, () => this.outputRuntime.commitFailureEvidence(failure))
        validateGvisorOutputFailureCommit(rawCommit, failure)
        throw error
      }
      ...
      const rawCommit = await boundedDurable("R3G-E durable positive output evidence", options.signal, () => this.outputRuntime.commitOutputEvidence(record))
      const commit = validateGvisorOutputBoundCommit(rawCommit, record)
      ...
      await observer?.onReceipt?.(receipt)
      throw new ExecutionFailedError(message, { cause: error })
    } finally {
