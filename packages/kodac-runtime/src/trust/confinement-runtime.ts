...
export const KDO_H4_R2C_PERMIT_FD: number = 5
...
function positiveBoundedInteger(value: unknown, label: string, maximum: number): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value <= 0 || value > maximum) throw new TypeError(`${label} must be an integer from 1 through ${maximum}`)
  return value
}
function nonNegativeBoundedInteger(value: unknown, label: string, maximum: number): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0 || value > maximum) throw new TypeError(`${label} must be an integer from 0 through ${maximum}`)
  return value
}
...
export function createConfinementExecutionAttempt(input: { executionIntentIdentity: string; nonce: string }): ConfinementExecutionAttempt {
  ...
  const base = Object.freeze({ version: KDO_H4_R2C_EXECUTION_ATTEMPT_VERSION, executionIntentIdentity, nonce })
  return Object.freeze({ ...base, executionAttemptIdentity: sha256(JSON.stringify(base)) })
}
export function validateConfinementExecutionAttempt(value: unknown): ConfinementExecutionAttempt {
  ...
  const rebuilt = createConfinementExecutionAttempt({ executionIntentIdentity: requireIdentity(record.executionIntentIdentity, "executionIntentIdentity"), nonce: boundedString(record.nonce, "execution-attempt nonce", MAX_NONCE_BYTES) })
  if (requireIdentity(record.executionAttemptIdentity, "executionAttemptIdentity") !== rebuilt.executionAttemptIdentity) throw new TypeError("execution-attempt identity mismatch")
  return rebuilt
}
export function createLocalWorkspaceRootIdentity(root: string): string {
  return sha256(`${KDO_H4_R2C_WORKSPACE_ROOT_VERSION}\n${absoluteCanonicalPath(root, "workspace root")}`)
}
export function createLauncherArtifactWriteProtection(input: { ownerUid: number; ownerGid: number; permissions: number; linkCount: number }): LauncherArtifactWriteProtection {
  ...
  if (record.ownerUid !== 0 || record.ownerGid !== 0) throw new TypeError("R2C launcher artifact must be owned by root:root")
  const permissions = nonNegativeBoundedInteger(record.permissions, "launcher artifact permissions", 0o777)
  if ((permissions & 0o022) !== 0) throw new TypeError("R2C launcher artifact must not be group- or world-writable")
  if ((permissions & 0o005) !== 0o005) throw new TypeError("R2C launcher artifact must be world-readable and world-executable for the non-root K2 host")
  if (record.linkCount !== 1) throw new TypeError("R2C launcher artifact must have exactly one hard link")
  return Object.freeze({ policy: KDO_H4_R2C_LAUNCHER_WRITE_PROTECTION, ownerUid: 0, ownerGid: 0, permissions, linkCount: 1 })
}
export function validateLauncherArtifactWriteProtection(value: unknown): LauncherArtifactWriteProtection {
  ...
  if (record.policy !== KDO_H4_R2C_LAUNCHER_WRITE_PROTECTION) throw new TypeError("launcher artifact write-protection policy mismatch")
  return createLauncherArtifactWriteProtection({ ownerUid: record.ownerUid as number, ownerGid: record.ownerGid as number, permissions: record.permissions as number, linkCount: record.linkCount as number })
}
export function createLauncherArtifactObservation(input: { launcherPath: string; sha256: string; sizeBytes: number; writeProtection: LauncherArtifactWriteProtection }): LauncherArtifactObservation {
  ...
  const base = Object.freeze({ version: KDO_H4_R2C_LAUNCHER_OBSERVATION_VERSION, launcherPath: absoluteCanonicalPath(record.launcherPath, "launcherPath"), sha256: requireIdentity(record.sha256, "launcher artifact sha256"), sizeBytes: positiveBoundedInteger(record.sizeBytes, "launcher artifact sizeBytes", KDO_H4_R2C_MAX_LAUNCHER_BYTES), writeProtection: validateLauncherArtifactWriteProtection(record.writeProtection) })
...
export function validateLauncherArtifactObservation(value: unknown): LauncherArtifactObservation {
  ...
  const rebuilt = createLauncherArtifactObservation({ launcherPath: absoluteCanonicalPath(record.launcherPath, "launcherPath"), sha256: requireIdentity(record.sha256, "launcher artifact sha256"), sizeBytes: positiveBoundedInteger(record.sizeBytes, "launcher artifact sizeBytes", KDO_H4_R2C_MAX_LAUNCHER_BYTES), writeProtection: validateLauncherArtifactWriteProtection(record.writeProtection) })
  if (requireIdentity(record.observationIdentity, "launcher observation identity") !== rebuilt.observationIdentity) throw new TypeError("launcher observation identity mismatch")
  return rebuilt
}
export function parseLinuxLandlockReadyRecord(value: string | Uint8Array): LinuxLandlockReadyRecord {
  const bytes = typeof value === "string" ? Buffer.from(value, "utf8") : Buffer.from(value)
  if (bytes.byteLength === 0 || bytes.byteLength > KDO_H4_R2C_READY_MAX_BYTES) throw new TypeError(`Landlock readiness record must contain 1..${KDO_H4_R2C_READY_MAX_BYTES} bytes`)
  for (const byte of bytes) if (byte > 0x7f) throw new TypeError("Landlock readiness record must be ASCII")
  ...
  const abi = Number(match[1]); if (!Number.isSafeInteger(abi) || abi <= 0) throw new TypeError("Landlock readiness ABI is invalid")
  const enforcement = requireObservedEnforcement(match[2])
  if (enforcement === "full" && abi < KDO_H4_R2B_LINUX_LANDLOCK_FULL_CLAIM_ABI) throw new TypeError("Landlock readiness cannot claim full below the local claim ABI")
  if (enforcement === "partial" && abi >= KDO_H4_R2B_LINUX_LANDLOCK_FULL_CLAIM_ABI) throw new TypeError("Landlock readiness cannot claim partial at or above the local claim ABI")
...
export function linuxLandlockReadyReason(ready: LinuxLandlockReadyRecord): string {
  ...
  return `Landlock READY abi=${abi} claim-set=${KDO_H4_R2B_LINUX_LANDLOCK_CLAIM_SET} enforcement=${requireObservedEnforcement(record.enforcement)}`
}
function evidenceSink(value: unknown): ConfinementEvidenceSink {
  ...
  if (descriptor === undefined || !("value" in descriptor) || typeof descriptor.value !== "function") throw new TypeError("confinement evidence sink commit must be a callable data property")
  return Object.freeze({ commit: descriptor.value as ConfinementEvidenceSink["commit"] })
}
export function createLinuxLandlockRuntimeConfig(input: { launcherPath: string; expectedLauncherSha256: string; evidence: ConfinementEvidenceSink; requiredEnforcement: "full" }): LinuxLandlockRuntimeConfig {
  ...
  return Object.freeze({ version: KDO_H4_R2C_RUNTIME_VERSION, launcherPath: absoluteCanonicalPath(record.launcherPath, "launcherPath"), expectedLauncherSha256: requireIdentity(record.expectedLauncherSha256, "expectedLauncherSha256"), evidence: evidenceSink(record.evidence), requiredEnforcement: "full" })
}
export function validateLinuxLandlockRuntimeConfig(value: unknown): LinuxLandlockRuntimeConfig {
  ...
  return createLinuxLandlockRuntimeConfig({ launcherPath: absoluteCanonicalPath(record.launcherPath, "launcherPath"), expectedLauncherSha256: requireIdentity(record.expectedLauncherSha256, "expectedLauncherSha256"), evidence: evidenceSink(record.evidence), requiredEnforcement: record.requiredEnforcement === "full" ? "full" : (() => { throw new TypeError("R2C required enforcement must be full") })() })
}
function recordBase(input: { executionAttempt: ConfinementExecutionAttempt; request: ConfinementRequest; enforcementEvidence: ConfinementEnforcementEvidence; launcherArtifact: LauncherArtifactObservation }) {
  ...
  if (request.mode !== "read-only" || request.scope.writePaths.length !== 0) throw new TypeError("R2C durable evidence requires a read-only request with no write paths")
  if (request.executionIntentIdentity !== executionAttempt.executionIntentIdentity) throw new TypeError("R2C request execution intent does not match execution attempt")
  if (enforcementEvidence.requestIdentity !== request.requestIdentity) throw new TypeError("R2C enforcement evidence request identity mismatch")
  if (enforcementEvidence.executionAttemptIdentity !== executionAttempt.executionAttemptIdentity) throw new TypeError("R2C enforcement evidence attempt identity mismatch")
  if (enforcementEvidence.enforcement !== "full" && enforcementEvidence.enforcement !== "partial") throw new TypeError("R2C durable observed evidence must be full or partial")
  const expectedBackend = createLinuxLandlockBackendDescriptor()
  if (enforcementEvidence.backend.backendIdentity !== expectedBackend.backendIdentity) throw new TypeError("R2C enforcement evidence backend identity mismatch")
  return Object.freeze({ version: KDO_H4_R2C_EVIDENCE_RECORD_VERSION, executionAttempt, request, enforcementEvidence, launcherArtifact, claimSet: KDO_H4_R2B_LINUX_LANDLOCK_CLAIM_SET })
}
export function createDurableConfinementEvidenceRecord(input: { executionAttempt: ConfinementExecutionAttempt; request: ConfinementRequest; enforcementEvidence: ConfinementEnforcementEvidence; launcherArtifact: LauncherArtifactObservation }): DurableConfinementEvidenceRecord {
  ...
  const base = recordBase({ executionAttempt: record.executionAttempt as ConfinementExecutionAttempt, request: record.request as ConfinementRequest, enforcementEvidence: record.enforcementEvidence as ConfinementEnforcementEvidence, launcherArtifact: record.launcherArtifact as LauncherArtifactObservation })
...
export function validateDurableConfinementEvidenceRecord(value: unknown): DurableConfinementEvidenceRecord {
  ...
  const rebuilt = createDurableConfinementEvidenceRecord({ executionAttempt: record.executionAttempt as ConfinementExecutionAttempt, request: record.request as ConfinementRequest, enforcementEvidence: record.enforcementEvidence as ConfinementEnforcementEvidence, launcherArtifact: record.launcherArtifact as LauncherArtifactObservation })
  if (requireIdentity(record.recordIdentity, "durable confinement record identity") !== rebuilt.recordIdentity) throw new TypeError("durable confinement record identity mismatch")
  return rebuilt
}
export function createDurableConfinementEvidenceCommit(record: DurableConfinementEvidenceRecord): DurableConfinementEvidenceCommit {
  ...
  const base = Object.freeze({ version: KDO_H4_R2C_EVIDENCE_COMMIT_VERSION, recordIdentity: validated.recordIdentity, durability: "durable" as const })
  return Object.freeze({ ...base, acknowledgmentIdentity: sha256(JSON.stringify(base)) })
}
export function validateDurableConfinementEvidenceCommit(value: unknown, record: DurableConfinementEvidenceRecord): DurableConfinementEvidenceCommit {
  ...
  if (requireIdentity(source.recordIdentity, "durable confinement commit record identity") !== expected.recordIdentity) throw new TypeError("durable confinement commit record identity mismatch")
  if (requireIdentity(source.acknowledgmentIdentity, "durable confinement commit acknowledgment identity") !== expected.acknowledgmentIdentity) throw new TypeError("durable confinement commit acknowledgment identity mismatch")
  return expected
}
export function createConfinementReceiptBinding(input: { record: DurableConfinementEvidenceRecord; commit: DurableConfinementEvidenceCommit }): ConfinementReceiptBinding {
  ...
  return Object.freeze({
    ...
    executionIntentIdentity: record.executionAttempt.executionIntentIdentity,
...
export function validateConfinementReceiptBinding(value: unknown): ConfinementReceiptBinding {
  ...
  exactKeys(record, ["version", "requestIdentity", "executionAttemptIdentity", "executionIntentIdentity", "backendIdentity", "enforcementEvidenceIdentity", "durableRecordIdentity", "durableCommitAcknowledgmentIdentity", "launcherArtifactSha256", "claimSet", "enforcement"], "confinement receipt binding")
  ...
  return Object.freeze({
    ...
    executionIntentIdentity: requireIdentity(record.executionIntentIdentity, "confinement binding execution intent identity"),
